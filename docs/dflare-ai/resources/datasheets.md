...
Single control plane manages Kubernetes and Slurm. Kubernetes handles containerized workloads via device plugins. Slurm handles batch workloads with GPU-aware scheduling using GRES. Both share the same underlying nodes, storage, and network.
